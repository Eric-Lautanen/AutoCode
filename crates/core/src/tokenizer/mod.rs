...
impl TiktokenTokenizer {
    ...
    fn for_model_family(model: &str) -> Option<Self> {
        ...
        let try_encoding = |name: &str| -> Option<&'static tiktoken::CoreBpe> {
        ...
        if model_lower.contains("gpt-4") || model_lower.contains("gpt-3.5") 
            || model_lower.contains("text-embedding-3") || model_lower.contains("text-embedding-ada") {
            return try_encoding("cl100k_base").map(|enc| Self { encoding: enc });
        }
        
        // Codex, code-davinci, older code models -> p50k_base
        if model_lower.contains("codex") || model_lower.contains("code-davinci") || model_lower.contains("code-cushman") {
