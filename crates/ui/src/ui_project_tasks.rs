...
pub fn show_window(ctx: &egui::Context, state: &mut AppState) {
    ...
    ctx.data_mut(|d| {
        d.insert_temp(egui::Id::new("project_tasks_open"), true);
    });

    let mut open = state.show_project_tasks;

    let content_rect = ctx.content_rect();
    let default_x = (content_rect.right() - 300.0 - 50.0).max(50.0);
    let default_y = content_rect.top() + 320.0;

    egui::Window::new("Project Tasks")
        .title_bar(false)
        .open(&mut open)
        .resizable(true)
        ...
        .show(ctx, |ui| {
            ...
            Frame::NONE
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        let title = if state.project_task_list.title.is_empty() {
            ...
            // Progress section.
            if !state.project_task_list.items.is_empty() {
                ui.add_space(4.0);
            ...
            Frame::NONE
                ...
                .show(ui, |ui| {
                    ...
                    if state.project_task_list.items.is_empty() {
                        ...
                        for item in &state.project_task_list.items {
    ...
    let all_done = !state.project_task_list.is_empty()
        && state
            .project_task_list
            .items
            .iter()
...
fn empty_state(ui: &mut egui::Ui) {
    ...
    ui.vertical_centered(|ui| {
        ...
        ui.add_space(8.0);
        ui.label(
            RichText::new("No project tasks yet")
                .size(12.5)
                .color(Palette::TEXT_MUTED),
        );
        ui.add_space(3.0);
        ui.label(
            RichText::new("Project tasks persist across")
                .size(10.5)
                .color(Palette::TEXT_MUTED),
        );
        ui.label(
            RichText::new("sessions for long-running goals")
                .size(10.5)
                .color(Palette::TEXT_MUTED),
        );
    });
