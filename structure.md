...
## ai/ — AI provider and chat orchestration (28 files, 6,981 lines)
...
ai/src/chat/mod.rs | 24 | Module index re-exporting public chat API
ai/src/chat/completion.rs | 688 | Sends messages, starts completions, handles handoff & auto-continue
ai/src/chat/errors.rs | 220 | Classifies transient/permanent errors, shortens OS errors, fixes params
ai/src/chat/polling/mod.rs | 116 | Buffer helpers, retry backoff, update_runtime/update_all frame loop
ai/src/chat/polling/stream.rs | 665 | Polls SSE provider stream, handles stalled/timeout, dispatches tool calls
ai/src/chat/polling/shell.rs | 287 | Live shell startup, streaming output, shell task polling
ai/src/chat/polling/tools.rs | 117 | Tool result collection, handoff detection, commit_tool_results
ai/src/chat/runtime.rs | 197 | ChatRuntime struct, NetworkStatus/blink, ToolResult, drain logic
ai/src/chat/session.rs | 166 | Session ensure/prepare/delete: seeds system prompt, loads from disk
ai/src/chat/session_ops.rs | 347 | Push messages, replay, trim RAM, push tool results, format context usage
ai/src/chat/tools.rs | 1,201 | Executes all 21 tools (file ops, shell, search, web, skills) on bg thread
...
ai/src/provider/client.rs | 397 | ProviderClient: sends requests, builds bodies, fetches models, counts tokens
ai/src/provider/http.rs | 868 | Low-level HTTP/TLS transport, SSE parsing, chunked decoding, timeouts
...
ai/src/provider/tool_defs.rs | 43 | Returns JSON tool definitions (21 tools) for the API
ai/src/provider/types.rs | 95 | Types: CompletionRequest, ApiMessage, ToolCall, ProviderEvent
ai/src/provider/web.rs | 464 | Web scraping: browser profiles, cookie jar, native_get/native_post

## autocode/ — Binary entry point (4 files, 23 lines)
...
## core/ — Shared types, state, storage, tokenizer, utils (34 files, 5,900 lines)
core/Cargo.toml | 8 | Crate manifest: deps (serde, scraper), workspace edition/version
core/src/lib.rs | 14 | Crate root: re-exports modules; doc comment describes core types, regex, tokenizer, fs, sysinfo
core/src/helpers/mod.rs | 37 | Re-exports all helper items (id, paths, regex, sanitize, serde_defaults, tokens, utils)
...
core/src/helpers/tokens.rs | 258 | Token estimation: heuristic for content, messages, tools, and full request JSON
core/src/helpers/utils.rs | 307 | Provider manifest loader, truncation helpers, token-usage display, sanitize display text
core/src/state/mod.rs | 19 | Re-exports all state types (AppState, ChatMessage, Project, Provider, Session, Todo, Secret)
core/src/state/app_state.rs | 559 | Top-level AppState: projects, providers, sessions, timeouts, rate limits, pending writes flush
core/src/state/chat.rs | 91 | ChatMessage, Role enum (System/User/Assistant/Tool/Error), ToolMeta for structured tool results
core/src/state/manifest.rs | 52 | ProviderManifest & ModelManifest deserialization from providers.json baked-in manifest
core/src/state/project.rs | 10 | Project struct: id, name, root_path, created_at, data_dir_name
core/src/state/provider.rs | 411 | ApiProvider config, ProviderKind (dynamic manifest-backed), ThinkingApi enum, model defaults
core/src/state/secret.rs | 55 | SecretString: zeroizes heap on drop via volatile writes, Debug redacts value
core/src/state/session.rs | 255 | Session struct: messages, token estimates, sampling params, PendingWrites rate-limiter
core/src/state/todo.rs | 115 | TodoList, ProjectTaskList, TodoItem, ProjectMeta types with progress/incomplete helpers
core/src/storage/mod.rs | 33 | Re-exports all storage items (chunked_jsonl, discovery, persistence, provider_file, session_io, etc.)
core/src/storage/app_storage.rs | 12 | Trait definitions for StorageLoad and AppStorage (load-only + read-write persistence)
core/src/storage/chunked_jsonl.rs | 287 | Chunked JSONL message persistence: append, read-all, load-before-id, truncate with crash-safe temp
core/src/storage/discovery.rs | 209 | Disk discovery: load/save project meta, discover projects/sessions, identity migration from legacy
...
core/src/storage/provider_file.rs | 237 | Serialize/deserialize providers.json with ModelEntry, round-trip to/from ApiProvider
core/src/storage/session_io.rs | 361 | Session dir management, atomic JSON writes, load/save/delete session meta & messages, orphan cleanup
core/src/storage/session_meta.rs | 104 | SessionMeta struct: session state snapshot persisted as session.json in subdirectory
core/src/storage/shell_task.rs | 82 | CRUD + pruning for per-project shell task JSON files on disk
core/src/tokenizer/mod.rs | 15 | Tokenizer trait, HeuristicTokenizer, tokenizer_for_model, offline_token_count
...
core/tests/stability.rs | 276 | Integration tests: long-running simulation (70 sessions, 7000 msgs) and crash-recovery round-trip

## fs/ — Filesystem exploration, shell, git, skills (17 files, 1,719 lines)
...
fs/src/explorer/tree.rs | 89 | Recursive project tree listing respecting .gitignore, depth 20, line counts

## ui/ — Desktop UI (egui/eframe) (46 files, 7,267 lines)
...
ui/src/app.rs | 494 | AutocodeApp: eframe::App glue — state/runtimes, save/load, top/left/center panels
...
ui/src/chat/input.rs | 331 | Input row: Multiline text + Send/Stop/TH/effort/todo toggles
...
ui/src/chat/panel.rs | 309 | Chat panel orchestrator: tabs → scroll → messages → streaming → input row
ui/src/chat/session.rs | 208 | Session lifecycle: save_old/load_new, purge missing, restore scroll offset
ui/src/chat/state.rs | 62 | ChatPanelState: input, display_buffer, scroll tracking, oldest_disk_id
ui/src/chat/tabs.rs | 199 | Session tab bar: activity spinner, click-to-switch, close with persist/delete
...
ui/src/explorer/state.rs | 52 | ExplorerPanelState: expanded/selected/content/image texture/rename/viewer scroll
ui/src/explorer/tree.rs | 292 | Recursive tree with git-status coloring, rename inline, context menu (copy/rename/delete)
ui/src/explorer/viewer.rs | 496 | Floating file viewer: image/text preview, line gutter, edit/save, close confirmation
ui/src/helpers/mod.rs | 21 | Helpers module root: re-exports diff/formatting/time/todo/tool_result/ui_id/widgets
...
ui/src/helpers/ui_id.rs | 143 | Unique ID generation for egui widgets + shared data-key constants
...
ui/src/settings/about.rs | 187 | About tab: version, providers, sysinfo display + OpenGL install guide
...
ui/src/settings/providers.rs | 740 | Providers tab: CRUD providers, model config (context/thinking/sampling), fetch models
ui/src/settings/session.rs | 83 | Session tab: RAM window size, completion/web/write rate limits
ui/src/settings/state.rs | 42 | SettingsState: Tab enum, fetched_models, rename/add buffers
...
ui/src/tasks/task_list.rs | 87 | Session/project task list windows: delegate to shared TodoWindow with per-scope config
ui/src/tasks/task_window.rs | 322 | Shared floating todo window: progress bar, item cards, auto-close on all done
...
ui/src/toolbar/meters.rs | 95 | Token usage bar + network status (blink dot, byte count, idle/stall)
ui/src/toolbar/mod.rs | 5 | Toolbar module root: re-exports layout::show
ui/src/toolbar/pickers.rs | 182 | ComboBox pickers for project, session, provider, model + New Project/Session buttons

---

All paths above are relative to `crates/`. Other notable directories at the project root:

| Path | Files | Lines | Role |
|------|-------|-------|------|
| skills/ | 76 | 15,738 | Skill markdown files bundled with the binary |
| assets/ | 11 | 9,715 | Screenshots, icons, installer assets |

| Crate | Files | Lines | Role |
|-------|-------|-------|------|
| autocode-ai (lib) | 28 | 6,981 | AI provider and chat orchestration |
| autocode (bin) | 4 | 23 | Windows binary entry point |
| autocode-core (lib) | 34 | 5,900 | Shared types, state, storage, tokenizer, utils |
| autocode-fs (lib) | 17 | 1,719 | Filesystem exploration, shell, git, skills |
| autocode-ui (lib) | 46 | 7,267 | Desktop UI (egui/eframe) |
| root Cargo.toml | 1 | 21 | Workspace manifest |
| **Total** | **130** | **21,911** | **123 .rs + 6 Cargo.toml + 1 app.rc** |
