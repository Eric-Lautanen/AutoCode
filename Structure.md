...
main.rs (51) — entry: debug init, rustls crypto install, eframe::run_native (1400x900, glow/wgpu)
...
helpers.rs (1764) — ID gen, token estimation, path resolution + traversal guard, fuzzy find-replace (6 strategies), similarity metrics, tiny regex engine, serde defaults, budget display, `unique_data_dir_name`
...
chat.rs (2764) — orchestration: send_message, start_completion, stream/tool/shell polling, 16 tool handlers, auto_name_session, handoff
...
ui_helpers.rs (422) — format_time, tool result summary/body, inline formatting, screen pixel sampling
